...
public class TwirlingRing extends CustomEffect {

	private boolean isReversed = false;
	private boolean isDirectional = false;
	
	public TwirlingRing(CustomParticle particle, double radius, double deltaRadius, double maxRotation, int maxDuration,
	...
	protected void tick(Location loc, int currentTick) {
		...
		spawnLoc.add(x * (scaling + 1), 0, z * (scaling + 1));
		...
		if(isDirectional) {
			...
			Vector previousPoint = new Vector(prevX, 0, prevZ);
			Vector currentPoint = new Vector(x, 0, z);
