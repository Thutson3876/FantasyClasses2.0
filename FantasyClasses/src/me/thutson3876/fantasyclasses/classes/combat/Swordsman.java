...
public class Swordsman extends AbstractAbility {
	...
	public void setDefaults() {
		...
		this.maximumLevel = 6;
		this.attackSpeed = new AttributeModifier(new NamespacedKey(FantasyClasses.getPlugin(), "swordsman"), speedMod, Operation.ADD_NUMBER,
	...
	public void onPlayerItemHeldEvent(PlayerItemHeldEvent e) {
		...
		AttributeInstance att = player.getAttribute(Attribute.ATTACK_SPEED);
		if (player.getInventory().getItem(e.getNewSlot()) != null && MaterialLists.SWORD.getMaterials()
			...
			if (!att.getModifiers().contains(attackSpeed)) {
				...
				player.getAttribute(Attribute.ATTACK_SPEED).addModifier(attackSpeed);
			...
			if (att.getModifiers().contains(attackSpeed)) {
				player.getAttribute(Attribute.ATTACK_SPEED).removeModifier(attackSpeed);
				return;
			}
		}
	}
	...
	public void applyLevelModifiers() {
		attackSpeed = new AttributeModifier(new NamespacedKey(FantasyClasses.getPlugin(), "swordsman"), speedMod * currentLevel, Operation.ADD_NUMBER,
				EquipmentSlotGroup.HAND);
	}
	
	@Override
	public void deInit() {
		if(player.getAttribute(Attribute.ATTACK_SPEED).getModifiers().contains(attackSpeed))
			player.getAttribute(Attribute.ATTACK_SPEED).removeModifier(attackSpeed);
	}
